...
///
/// Tandem patch (vs upstream sonora-aec3 0.1.0): no-op on shrink
/// (new_size <= old_size). Upstream uses `&mut h[old_size..new_size]`
/// which panics with "slice index starts at N but ends at M" when the
/// filter shrinks during a smooth resize (update_size interpolates the
/// current size down toward a smaller target). The reference WebRTC C++
/// implementation iterates `for (k = old_size; k < new_size; ++k)`,
/// which is a no-op on shrink — matching that here.
