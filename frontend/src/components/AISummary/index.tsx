...
export const AISummary = ({ summary, status, error, onSummaryChange, onRegenerateSummary, meeting }: Props) => {
  ...
  const renderErrorState = () => (
    <div className="w-full p-4 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-lg">
      <div className="flex items-center mb-2">
        <ExclamationTriangleIcon className="h-5 w-5 text-red-500 mr-2" />
        <h3 className="text-red-700 dark:text-red-400 font-medium">Error Generating Summary</h3>
      </div>
      <p className="text-red-600 dark:text-red-400 text-sm">{error}</p>
      <p className="text-red-500 dark:text-red-500/70 text-xs mt-2">Please check your model configuration and API keys, or try again.</p>
    </div>
  );
    ...
    <div className="relative">
      ...
      {contextMenu.visible && selectedBlocks.length > 0 && (
        ...
        >
          ...
          <button
            className="w-full px-4 py-2 text-left hover:bg-accent text-red-600 flex items-center space-x-2"
