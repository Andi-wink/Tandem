  ...
  const handleBrowse = async () => {
    try {
      ...
      if (dbPath) {
        ...
        setErrorMessage('No database found at selected location. Please select the Tandem folder, backend folder, or the database file directly.');
  ...
  return (
    <Dialog open={isOpen} onOpenChange={() => {}}>
      <DialogContent className="sm:max-w-[600px]" onPointerDownOutside={(e) => e.preventDefault()}>
        <DialogHeader>
          <DialogTitle className="text-2xl">Welcome to Tandem!</DialogTitle>
          <DialogDescription className="text-base pt-2">
            Do you have data from a previous Tandem/Meetily installation?
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">
              Select your previous Tandem/Meetily folder, backend directory, or database file:
            </p>

            <button
              onClick={handleBrowse}
              disabled={isLoading}
              className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:bg-muted-foreground disabled:cursor-not-allowed transition-colors"
            >
