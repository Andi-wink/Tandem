...
h3 { font-size: 0.95rem; margin: 24px 0 8px; }
.board-count { color: var(--muted); font-size: 0.85rem; margin: 0 0 12px; }
figure { margin: 0; }
...
export function generateHandoverHtml(data: HandoverData, images: Map<string, string>): string {
  ...
  // The board is the state the call ended in rather than a moment inside it, so it follows the
  // timeline. Its PNG inlines like every other image; an unreadable one is called out, not dropped.
  if (data.whiteboard) {
    ...
    if (text) {
      parts.push('<h3>Text on the board</h3>');
