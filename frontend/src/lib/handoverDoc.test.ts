...
describe('generateHandoverMarkdown', () => {
  ...
  it('renders the whiteboard after the timeline when the meeting had a board', () => {
    const md = doc({
      whiteboard: { pngPath: `${FOLDER}\\whiteboard.png`, shapeCount: 7, text: 'pricing\nnext steps' },
    });
    expect(md).toContain('## Whiteboard');
    expect(md).toContain('7 shapes drawn during the meeting');
    expect(md).toContain('![Whiteboard](whiteboard.png)');
    expect(md).toContain('### Text on the board');
    expect(md).toContain('pricing\nnext steps');
    expect(md.indexOf('## Timeline')).toBeLessThan(md.indexOf('## Whiteboard'));
    expect(md.indexOf('## Whiteboard')).toBeLessThan(md.indexOf('*Generated by Tandem'));
  });

  it('counts the whiteboard in the captured line and singularises one shape', () => {
    const md = doc({ whiteboard: { pngPath: `${FOLDER}\\whiteboard.png`, shapeCount: 1 } });
    expect(md).toContain('1 clipboard item, 1 whiteboard');
    expect(md).toContain('1 shape drawn during the meeting');
  });

  it('omits the board text sub-heading when the board carries no text', () => {
    const md = doc({ whiteboard: { pngPath: `${FOLDER}\\whiteboard.png`, shapeCount: 3, text: '   ' } });
    expect(md).toContain('## Whiteboard');
