...
impl SidecarManager {
    ...
    fn resolve_helper_binary() -> Result<PathBuf> {
        ...
        //    Falls back to old MEETILY_LLAMA_HELPER for backwards compatibility
        if let Ok(env_path) = std::env::var("TANDEM_LLAMA_HELPER")
            .or_else(|_| std::env::var("MEETILY_LLAMA_HELPER"))
        {
            if !env_path.is_empty() {
                let path = PathBuf::from(env_path);
                if path.exists() {
                    log::info!("Using llama-helper from env var: {}", path.display());
                    return Ok(path);
                }
            }
        }
        ...
        Err(anyhow!(
            "llama-helper binary not found. Build with 'cd llama-helper && cargo build --release' or set TANDEM_LLAMA_HELPER env var."
        ))
    }
