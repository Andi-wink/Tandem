...
pub fn get_default_recordings_folder() -> PathBuf {
    ...
    {
        // Windows: %USERPROFILE%\Music\tandem-recordings
        if let Some(music_dir) = dirs::audio_dir() {
            ...
            dirs::document_dir()
                ...
                .join("tandem-recordings")
        }
    }

    #[cfg(target_os = "macos")]
    {
        // macOS: ~/Movies/tandem-recordings
        if let Some(movies_dir) = dirs::video_dir() {
            movies_dir.join("tandem-recordings")
        } else {
    ...
    {
        // Linux/Others: ~/Documents/tandem-recordings
        dirs::document_dir()
            .unwrap_or_else(|| PathBuf::from("."))
            .join("tandem-recordings")
    }
        ...
        Some(file_path) => {
            match file_path.into_path() {
                Ok(path) => {
                    ...
                    info!("User selected folder: {}", p);
                    Ok(Some(p))
                }
                Err(e) => {
                    warn!("Failed to convert selected path: {}", e);
                    Ok(None)
                }
            }
        }
