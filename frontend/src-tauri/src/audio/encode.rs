...
pub fn encode_single_audio(
    ...
    command
        .args([
            ...
            output_path.to_str().ok_or_else(|| {
                anyhow::anyhow!("Output path contains invalid UTF-8 characters: {:?}", output_path)
            })?,
        ])
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
    ...
    let mut ffmpeg = command.spawn().map_err(|e| {
        anyhow::anyhow!("Failed to spawn FFmpeg process: {}. Ensure FFmpeg is installed and accessible.", e)
    })?;
    debug!("FFmpeg process spawned");
    let mut stdin = ffmpeg.stdin.take().ok_or_else(|| {
        anyhow::anyhow!("Failed to open FFmpeg stdin pipe")
    })?;
