...
Worktree `D:\Dev-projects\Tandem-handover-canvas`. The handover document now carries the board drawn during the call: a `## Whiteboard` section after the timeline with the shape count, the PNG (relative in [HANDOVER.md](frontend/src/lib/handoverDoc.ts), inlined as a data URI in [HANDOVER.html](frontend/src/lib/handoverHtml.ts)) and the board text, normalised by [boardText.ts](frontend/src/lib/boardText.ts). `countShapes` in [whiteboardSnapshot.ts](frontend/src/lib/whiteboardSnapshot.ts) keeps empty boards out, and [useWhiteboardPersistence](frontend/src/hooks/useWhiteboardPersistence.ts) stops writing blank boards in the first place. The board is read from the meeting folder only, never from the live canvas, so a handover for a past meeting cannot pick up the board that happens to be open. Gates: `pnpm tsc --noEmit` clean, `pnpm test` 525/525 across 41 files. One adversarial QA pass found three real defects (live-canvas cross-contamination, verbatim board text, section dropped on a failed PNG embed), all fixed in `edd483c`.
...
- [ ] LIVE-RUNTIME needed: with a board open on the canvas, generate a handover for a DIFFERENT, older meeting and confirm the open board is not written into that meeting's folder or its HANDOVER.html.
