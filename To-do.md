...
- [ ] I3b misfile correction — BUILDING (wf_4efc902e-412): "Filed under X" row on meeting details with Move-to-project (ProjectPickerDialog + relocate_meeting_folder, undo restores previous location+association) and Unfile (back to default recordings dir). MOVE not delete. UI must disable actions for the live recording; manual corrections teach frecency. Opus build + adversarial data-safety QA + fix + final gate. Rejected alternatives logged: per-project bulk cleanup view (deferred as the future "all Acme calls" surface), palette-only "move last meeting" (transient, misses late discoveries).
