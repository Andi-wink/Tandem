...
- [ ] I3 calendar-driven routing — BUILDING (wf_faf054c1-846, plan amended with user's testing requirements): base plan + R1 ask-on-ambiguity (multiple candidate folders -> chooser seeded with ranked candidates; single strong match -> apply+undo; explicit agenda-row click never re-asks), R2 clients-root discovery (D:/Dev-projects/Client_projects subfolders as candidates, configurable setting, adopt-on-pick), R3 artifacts always in <project>/.tandem/ with call-date-stamped folder names (existing "Meeting <ts>" convention; relocation deferred to post-recording-stop, undo-aware). Two parallel adversarial skeptics + fix pass + final gate.
- [ ] I3b misfile correction (user-picked winner of 3 options, QUEUED behind the running I3 build to avoid same-file collisions): "Filed under X" control on meeting details with Move-to-project (ProjectPicker; physically relocates the dated .tandem meeting folder + updates SQLite folder_path + whiteboard mirror) and Unfile (back to default location). MOVE not delete by design. Reuses I3's Rust relocation command + undo semantics. Build with Opus + adversarial QA (data-loss lens). Rejected alternatives logged: per-project bulk cleanup view (deferred as the future "all Acme calls" surface), palette-only "move last meeting" (transient, misses late discoveries).
