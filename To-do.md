...
- [ ] I5b handover mode — IN FLIGHT (workflow running). Reminder during a live recording = non-intrusive toast + native notification only (no focus steal), "Wrap up and start next" stops+saves+summarizes the current meeting, then starts the next named after the calendar invite. Spec: I5b addendum in [research/meeting-reminder-popup-I5-plan.md](research/meeting-reminder-popup-I5-plan.md).
