...
**Tier 1, before the next German call**
- [x] **Pre-record consent gate — SHIPPED** (commit `a498c3f`). Enforced in Rust inside `start_recording` ([consent.rs](frontend/src-tauri/src/consent.rs)), not in React, so the record button, sidebar event, calendar seed, Alt+Shift+E and the tray menu are all covered and a forgotten wiring fails closed. New `consent_log` table records who consented, how, when, jurisdiction, language and engine (local vs cloud, because that is the § 201 Abs. 1 Nr. 2 limb), with `granted_at` and `recording_started_at` as separate columns. A grant is consumed on use and expires after 10 minutes. [ConsentDialog.tsx](frontend/src/components/ConsentDialog.tsx) names each participant (BVerfGE 106, 28) and escalates wording for GR/DE/PT/CH/FR. The three duplicated start paths in [useRecordingStart.ts](frontend/src/hooks/useRecordingStart.ts) collapsed into one; a blocked start is parked and replayed after consent. Settings toggle defaults ON. 6 new Rust tests.
  - [ ] Follow-up: surface the consent log in Settings (the `list_consent_log` command exists and is unused by any UI).
  - [ ] Follow-up: consent is currently per recording, not per meeting resumption. Decide whether resuming after a stop needs a fresh grant.
- [ ] **Make deletion delete, and file it as a bug first** (it is missing from `bug_list.json`). [meeting.rs#L569-L610](frontend/src-tauri/src/database/repositories/meeting.rs#L569-L610) is four SQL DELETEs with no filesystem call, though the row stores `folder_path`. Meanwhile the UI claims "All associated data has been removed" ([Sidebar/index.tsx#L381-L383](frontend/src/components/Sidebar/index.tsx#L381-L383)). Sweep the 138 known orphaned folders (2.0 GB).
- [ ] **Force local transcription for German/confidential calls.** No per-meeting engine switch exists ([setting.rs#L156](frontend/src-tauri/src/database/repositories/setting.rs#L156) is a single-row global). Until one ships, set it manually: sending German call audio to a US vendor is the § 201 Abs. 1 Nr. 2 limb plus a Chapter V transfer.

**Tier 2, German accuracy (cheapest first, and the diagnosis changed)**
- [x] **Persist the language preference — SHIPPED** (commit `a498c3f`). Stored in `app_settings`, hydrated into the in-memory cache at every `AppState` registration so a recording started before the UI mounts sees the pinned language. The cache stays because the transcription hot path reads it synchronously.
  - [ ] Still open: **pin it per meeting**. There is no `language` column on `meetings`, so a past meeting does not record what language it was transcribed in. Deferred because it touches the `Meeting` struct and every `SELECT *`; the consent log captures language per recording in the meantime.
- [x] **Language hint wired into every engine that drops it — SHIPPED** (commits `8d11103`, and `a093d18` on `feature/scribe-realtime-ws`). Fixed: the live Parakeet arm (never read the preference at all), the Parakeet provider path (warned and discarded), the Mistral chat route (returned before language handling, so German audio ran under an English instruction), the Mistral STT route (POSTed the literal `language=auto`), the Whisper English initial prompt (now language-conditional), and the realtime WS session (passed `None` against a working `build_url`). Also: the hallucination filter was English-only, so German artefacts passed through while English ones were dropped, and `ConfigContext` defaulted to `'auto-translate'` where Rust defaults to `'auto'`.
  - Honest limitation: **Parakeet cannot be steered.** TDT v3 auto-detects and exposes no language token, so the hint gates the English-only post-processing rather than the acoustic decode. For a genuinely language-pinned local decode, use Whisper or swap the model.
- [ ] **Fix the scoring before believing any German number.** Add ß/ss, umlaut and hyphen equivalence to `normalize()` ([run_tandem_parakeet.py#L463-L471](audio_testing/run_tandem_parakeet.py#L463-L471)), add a `lang` field to `clips_index.json`, re-score clip_07 with `de` pinned. Expect the number to roughly halve with no pipeline change. **The 57.04% headline came partly from a run whose normaliser dropped fillers "uh/um/er", and `er` is a German pronoun.**
- [x] ~~**Do not tune VAD first.** clip_07 is 42% substitutions vs 6% deletions...~~ **WITHDRAWN 2026-08-11, this was measured on the wrong engine.** Those figures come from [baseline_summary.md](audio_testing/results/baseline_summary.md), whose line 3 names the engine as **openai-whisper small**, not Parakeet. Recomputed against the pipeline the gate scores: **German is deletion-dominant** (hyp 107 words vs 142-word ref, 31 of 37 deletions in 4 contiguous blocks incl. a 14-word drop), and English clip_10 is D 1.5%, not 19.7%. The German problem is **content loss**, so the VAD/buffer path is back on the table and normalisation is worth ~4.9 pp of 53.5 pp. Tracked in [PARAKEET-BACKLOG.md](audio_testing/PARAKEET-BACKLOG.md) as P1b.
- [ ] **German measurement is underpowered: N=142 words, Wilson 95% CI [45.33%, 61.52%], MDE ~12 pp.** Nothing under ~10 pp is detectable. Expanding the German clip set (backlog P1a) blocks all further German work.
- [ ] **The compiled domain corrector fires on 0 of 16 buffers across all 5 clips.** Only the `N A N` -> `n8n` phrase rule ever fires (+1.875 pp on clip_04). Decide whether to delete the 0.86-similarity fuzzy pass rather than extend it (backlog P4).
- [ ] **Measure the code-switching cost of pinning `de`.** DECM (LREC-COLING 2024): forcing Whisper's German prefix moved WER on *embedded English words* from 34.68% to 49.49% while German words stayed at 15.02%. Pinning fixes the catastrophic script-flip and costs something on Denglisch. Net likely positive, but measure it.
- [x] **German PII anonymisation — SHIPPED** (commit `2ecc3e3`). Loads every installed spaCy model, resolves the analysis language per request (frontend supplies it automatically from the Rust preference), sniffs German when the language is `auto`, and registers a German phone recognizer with German context words. Measured on one German sentence, the English model invented two people out of "Guten Tag" and "Meine Nummer" and **leaked the phone number**; the German model gets the name boundary right and surrogates the number. Both models pinned in `requirements.txt`. 13 new tests.
- [ ] **Correction to the 2026-08-11 provider doc: do not adopt Voxtral Realtime for German.** Independently on the Open ASR Leaderboard it is 6.56% German mean, below Whisper large-v3 (4.26) and Parakeet v3 (4.20), at ~1/60th throughput, with no language param, no keyterm biasing and no diarization on the realtime API. The 6.19% FLEURS figure was vendor-published read speech. Better local candidate: `primeline/parakeet-primeline` (CC-BY-4.0, 600M, ~2 GB). And note **Parakeet TDT v3 already ships and already covers German** — we simply never tell it which language.
- [ ] **Planning band for real German calls is 12-20% WER, not 4-6%.** Every published German number is read speech; read-to-spontaneous degradation runs ~5x. The Artificial Analysis streaming board used in the provider doc is **English-only**. No public German meeting corpus exists; proxies are TalkBank CallHome German and DECM.

**Tier 3, paperwork**
- [ ] **Appoint an Art 27 EU representative in Germany**, in writing, named in the privacy notice. The Art 27(2)(a) "occasional" exemption fails: EDPB Guidelines 3/2018 read it as processing "outside the regular course of business", and recording client calls is the regular course of ours. Few hundred euros a year. Probably currently unmet.
- [ ] The one-stop-shop does **not** apply to a non-EU controller (WP244 rev.01 §§ 2.3, 3.3), so a complaint goes to the client's Landesdatenschutzbehörde, not the ICO.
- [ ] Write the Art 6(1)(f) LIA and the client-facing Art 13 notice; put the German consent script in the engagement letter. (Still-open F6 from the GDPR review.)
- [ ] **Deliver clients summaries, not attributed verbatim transcripts of their own employees.** That is what triggers § 87 Abs. 1 Nr. 6 BetrVG works-council co-determination on their side and stalls the deal. Third-party ownership is no defence (BAG 27.01.2004 – 1 ABR 7/03), and the threshold is low (BAG 16.07.2024 – 1 ABR 16/23: co-determined even where nothing is recorded or stored).
- [ ] **Never enable voice- or video-based emotion, mood or engagement scoring.** AI Act Art 5(1)(f), 7% / €35m tier. Text-only sentiment is expressly outside it (Commission prohibited-practices Guidelines para 251). A plain notetaker is not high-risk, and Annex III is now deferred to 2 December 2027 by Reg. (EU) 2026/1744.
...
- [x] **Voxtral self-hosting feasibility: GO WITH PREREQUISITES, but NOT via vLLM** ([voxtral-selfhost-feasibility-2026-08-11.md](research/voxtral-selfhost-feasibility-2026-08-11.md), commit 468a738). Model is not gated (Apache 2.0, no token), but the repo ships the same weights twice, so a naive `*.safetensors` pull costs 17.72 GB not 8.86 GB. VRAM was never the constraint: 18.1 GB free vs 8.86 GB of BF16 weights.
- [ ] **Do not adopt vLLM for the live path.** `/v1/realtime` exists (v0.16.0+, recipe-blessed at 0.20.0, needs `--tokenizer-mode mistral`) but open issue #47614 reproduces a blank-token rut that silently mutes transcription for 1 to 20 minutes **and loses that audio**, and #36015 reports a silent hang after ~10 min of continuous streaming. Five fix PRs unmerged for 2 to 3 months; the official recipe says to restart sessions periodically. This is the same failure mode as the 43%-loss bug below, so adopting it would reintroduce the bug we are trying to fix.
- [ ] Recommended runtime instead: **native Windows CUDA, no WSL2, no Docker.** Either `transformers` 5.2.0, which ships a native streaming `VoxtralRealtimeForConditionalGeneration` + `TextIteratorStreamer` (experimental; the HF card's "only supported in vLLM" line is stale and contradicts its own runtime list), or GGUF via `audio.cpp`/`transcribe.cpp`, the same ggml shape Tandem already links through whisper-rs. `nvcc` (CUDA 13.1) is already on PATH. llama.cpp proper is a hard no: the Voxtral Realtime issue is closed as not planned. Ollama, LM Studio, SGLang, LocalAI, Speaches all no.
- [ ] **Real-time factor on a 3090 is still unknown.** NOT FOUND in any published benchmark, and deliberately not estimated. The "12.5 tok/s" figure is the model's frame rate, i.e. exactly RTF 1.0 for a single stream, not a capacity measurement. The only concurrency data point is a community 4090 report whose author judged it insufficient for production. **This is the question that decides the whole approach and it needs a local measurement.**
- [ ] [audio_testing/run_voxtral_realtime_wer.py](audio_testing/run_voxtral_realtime_wer.py) was **never executed or verified**, and review found a core defect: it sends `model` at the top level of `session.update` instead of nested under `session`, and never declares an audio format or sample rate while feeding 16 kHz PCM16. If the server assumes OpenAI's 24 kHz default it will produce a plausible but meaningless WER. Five lesser defects too, including a `--dump-disagreements` flag the docstring promises but argparse never defines. Fix before any run.
...
- [ ] `docker_data.vhdx` is **106.4 GB** (written 2026-08-11 11:50). Inspect with `docker system df` once the daemon is up, then `docker image prune -a`. WSL2 vhdx files never auto-shrink, so reclaiming space needs a separate compact step. Neither vhdx is actually sparse despite `sparseVhd=true` in `.wslconfig`.
- [ ] **~146 GB reclaimable on C:, worth doing regardless of Voxtral** (from [voxtral-selfhost-feasibility-2026-08-11.md](research/voxtral-selfhost-feasibility-2026-08-11.md)): move Docker's disk-image location to D: (106.4 GB), set `HF_HOME=D:\hf-cache` and move the existing cache (~27 GB), delete `Windows.old` (12.1 GB). Takes C: from 45.4 GB to roughly 191 GB free.
- [ ] Closing Antigravity frees **10.8 GB** across 100 processes. Mandatory before any local-inference run: every runtime path fails at the current 2.97 GB free.
