...
- [ ] **Pre-record consent gate.** Modal before `start_recording`, not the current toast after it ([useRecordingStart.ts#L138](frontend/src/hooks/useRecordingStart.ts#L138) awaits backend start; toast only at [#L157](frontend/src/hooks/useRecordingStart.ts#L157) — that is the legally dangerous ordering). Persist a `consent_log` row: meeting id, timestamp, participants, method, jurisdiction. Today the "I've Notified Participants" click fires an analytics event and nothing else ([recordingNotification.tsx#L46-L47](frontend/src/lib/recordingNotification.tsx#L46-L47)). Also gate the `sessionStorage.autoStartRecording` path ([useRecordingStart.ts#L172](frontend/src/hooks/useRecordingStart.ts#L172)).
...
- [ ] **Persist the language preference and pin it per meeting.** `LANGUAGE_PREFERENCE` is a RAM-only process global ([lib.rs#L69-L71](frontend/src-tauri/src/lib.rs#L69-L71)) with no column in any of the 14 migrations. Picking German silently reverts to auto-detect on restart.
- [ ] **Wire the hint into the four places that drop it:** the live Parakeet arm ([worker.rs#L587-L588](frontend/src-tauri/src/audio/transcription/worker.rs#L587-L588), which never even reads the preference), the realtime WS session ([recording_commands.rs#L603-L604](../Tandem-scribe-rt/frontend/src-tauri/src/audio/recording_commands.rs#L603-L604) passes `None` against a `build_url` that already supports `&language_code=`), Mistral's sentinel handling, and the unconditional **English** initial prompt on Whisper ([whisper_engine.rs#L572-L575](frontend/src-tauri/src/whisper_engine/whisper_engine.rs#L572-L575)).
...
- [ ] **Do not tune VAD first.** clip_07 is 42% substitutions vs 6% deletions; VAD clipping produces deletions. Substitution-dominance means wrong-language decoding, not clipped audio. (English clip_10 shows the opposite: 19.7% Del, 1.4% Sub.)
...
- [ ] **Install the German spaCy model and wire `TANDEM_PII_LANGUAGE`.** [anonymizer.py#L37-L46](backend/app/anonymizer.py#L37-L46) hardcodes `en_core_web_sm`, so the anonymiser is inert on exactly the calls that need it.
