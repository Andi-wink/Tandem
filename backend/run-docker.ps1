...
$AppProjectName = "tandem-backend"
$AppContainerName = "tandem-backend"
$DefaultPort = 8178
$DefaultAppPort = 5167
...
function Invoke-StartCommand {
    ...
    $composeArgs += @("whisper-server", "tandem-backend")
    ...
    if ($DryRun) {
        ...
        if ($detach) {
            ...
            & docker-compose -f $ComposeFile --profile default up -d whisper-server tandem-backend
            ...
            & docker-compose -f $ComposeFile --profile default up whisper-server tandem-backend
...
function Invoke-LogsCommand {
    ...
    $serviceMap = @{
        ...
        "app" = "tandem-backend"
        "backend" = "tandem-backend"
        "meeting" = "tandem-backend"
    }
    ...
    if ($service) {
        ...
        $validServices = @("whisper-server", "tandem-backend")
        if ($service -notin $validServices) {
            Write-Warn "Unknown service: $service"
            Write-Info "Available services: whisper-server, tandem-backend"
        ...
        $logArgs += @("whisper-server", "tandem-backend")
    }
