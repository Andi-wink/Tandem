...
function Find-ExistingDatabases {
    ...
    $commonPaths = @(
        "$env:USERPROFILE\.tandem\meeting_minutes.db",
        "$env:USERPROFILE\Documents\tandem\meeting_minutes.db",
    ...
    if ($env:HOMEBREW_PREFIX) {
        $commonPaths += "$env:HOMEBREW_PREFIX/Cellar/tandem-backend/*/backend/meeting_minutes.db"
    }
