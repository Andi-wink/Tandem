...
**All colors and brand-specific styles live in one file:** `backend/skills/excalidraw/references/color-palette.md`. Read it before generating any diagram and use it as the single source of truth for all color choices — shape fills, strokes, text colors, evidence artifact backgrounds, everything.
...
Colors encode information, not decoration. Every color choice should come from `backend/skills/excalidraw/references/color-palette.md` — the semantic shape colors, text hierarchy colors, and evidence artifact colors are all defined there.
...
See `backend/skills/excalidraw/references/element-templates.md` for copy-paste JSON templates for each element type (text, line, dot, rectangle, arrow). Pull colors from `backend/skills/excalidraw/references/color-palette.md` based on each element's semantic purpose.
...
```bash
python backend/skills/excalidraw/render_excalidraw.py <path-to-file.excalidraw>
```
